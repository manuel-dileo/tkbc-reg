...
def to_cmd(c, _path=None):
    command = f'PYTHONPATH=. python3 ../learner.py --rank 156 --emb_reg 1e-2 --time_reg 1e-2 '\
        f'--dataset yago15k '\
        f'--model TNTComplEx '\
        f'--rank {c["rank"]} --emb_reg {c["emb_reg"]} --no_time_emb --time_reg {c["time_reg"]} --time_norm {c["time_norm"]}'
    return command


def to_logfile(c, path):
    outfile = "{}/yago15k.{}.log".format(path, summary(c).replace("/", "_")) #change here for other datasets
    return outfile
...
def main(argv):
    hyp_space_1 = dict(
        rank=[128, 256],
        emb_reg=[1, 1e-1, 1e-2, 1e-3],
        time_reg=[1, 1e-1, 1e-2, 1e-3],
        time_norm=['Lambda3','N3','L1','L2','F2']
    )
